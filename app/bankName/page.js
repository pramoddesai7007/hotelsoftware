...
const BankNameList = () => {
  ...
  useEffect(() => {
    const fetchBankNames = async () => {
      try {
        const response = await axios.get('https://backendback.vercel.app/api/bankName/bankNames');
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      await axios.patch(`https://backendback.vercel.app/api/bankName/bankNames/${selectedBankName._id}`, {
  ...
  const handleDeleteSubmit = async () => {
    try {
      await axios.delete(`https://backendback.vercel.app/api/bankName/bankNames/${selectedBankName._id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post('https://backendback.vercel.app/api/bankName/bankNames', { bankName: newBankName });
  ...
  return (
    <>
      <Navbar />
      <div className="max-w-5xl mx-auto bg-white p-4 rounded-lg shadow-md mt-20">
        <h2 className='text-center font-bold'>Bank Name Master</h2>
        <form onSubmit={handleSubmit} className="mb-4">
          <div className="mb-4">
            <label htmlFor="newBankName" className="block text-sm font-medium text-gray-600">
              Add Bank Name:
            </label>
            <input
              type="text"
              ...
              className="mt-1 p-2 border rounded-md lg:w-1/2 focus:outline-none focus:ring focus:border-blue-300"
            />
          </div>
            ...
            <table className="min-w-full">
              ...
              <tbody className="text-md font-sans font-semibold">
                {bankNames.map((bankName, index) => (
                  ...
                  >
                    <td className=" p-2 text-left text-gray lg:pl-32 pl-4">{bankName.bankName}</td>

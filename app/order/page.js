...
const Billing = ({ tableId, acPercentage }) => {
    ...
    const fetchTastes = async () => {
      try {
        const response = await axios.get(
          "https://backendback.vercel.app/api/taste/tastes"
        );
  ...
  const modifiedCurrentOrder = currentOrder.map((orderItem) => {
    ...
    const selectedTaste =
      selectedTasteId === "other"
        ? { _id: "other", taste: newTastes[orderItem._id] || "" }
        : tastes.find((taste) => taste.taste === selectedTasteId) || { _id: null, taste: selectedTasteId }; // Include selectedTasteId as taste if not "other" or not found in tastes
  ...
  const fetchWaitersList = async () => {
    try {
      const response = await axios.get("https://backendback.vercel.app/api/waiter");
  ...
  useEffect(() => {
    const fetchGreetings = async () => {
      try {
        const response = await axios.get(
          "https://backendback.vercel.app/api/greet/greet"
        );
  ...
  useEffect(() => {
    ...
    const fetchNextOrderNumber = async () => {
      try {
        const response = await axios.get(
          "https://backendback.vercel.app/api/order/get-next-order-number"
        );
  ...
  const fetchLastAllOrders = async () => {
    try {
      const ordersResponse = await axios.get(
        "https://backendback.vercel.app/api/order/latest-orders"
      );
      ...
      const tableNamesPromises = orders.map(async (order) => {
        const tableResponse = await axios.get(
          `https://backendback.vercel.app/api/table/tables/${order.tableId}`
        );
  ...
  const saveBill = async () => {
    try {
      ...
      const existingBillResponse = await axios.get(
        `https://backendback.vercel.app/api/order/order/${tableId}`
      );
      ...
      if (existingBill && existingBill.length > 0) {
        ...
        const updateResponse = await axios.patch(
          `https://backendback.vercel.app/api/order/update-order-by-id/${orderIdToUpdate}`,
          orderData
        );
      ...
      const kotData = {
        ...
        items: [...uniqueItemsSet].map((itemName, index) => {
          ...
          return {
            name: orderItem.name,
            quantity: orderItem.quantity,
            taste: tasteInfo && tasteInfo.selectedTaste
      ...
      const KOTResponse = await axios.post(
        `https://backendback.vercel.app/api/kot/kotOrder/${tableId}`,
        kotData
      );
  ...
  const saveKot = async () => {
    try {
      ...
      const existingKOTResponse = await axios.get(
        `https://backendback.vercel.app/api/kot/kot/${tableId}`
      );
        ...
        <html>
          ...
          .map(
            (orderItem, index) => `
                <tr>
                  <td>${index + 1}</td>
                  <td class="kot-item-name">${orderItem.name}${orderItem.taste
                ? `<br>Taste - ${orderItem.taste}`
                : ""
              }</td>
                  <td>${orderItem.quantity}</td>
                </tr>
...
<p style="${!hotelInfo || !hotelInfo.sacNo ? "display: none;" : ""}">SAC No: ${hotelInfo ? hotelInfo.sacNo : "SAC No Not Found"
  ...
  useEffect(() => {
    ...
    // Fetch categories
    axios
      .get("https://backendback.vercel.app/api/main")
      .then((response) => {
    ...
    // Fetch products
    axios
      .get("https://backendback.vercel.app/api/menu/menus/list")
      .then((response) => {
        console.log(response.data);
    ...
    if (tableId) {
      axios
        .get(`https://backendback.vercel.app/api/table/tables/${tableId}`)
        .then((response) => {
  ...
  useEffect(() => {
    ...
    if (selectedCategory) {
      axios
        .get(`https://backendback.vercel.app/api/menu/${selectedCategory._id}`)
        .then((response) => {
          console.log(response.data);
          const menusArray = response.data || []; // Ensure menus is an array
          setMenus(menusArray);
  ...
  const handleCategoryClick = (category) => {
    ...
    if (category === null) {
      ...
      // Fetch menus based on the selected category
      axios
        .get(`https://backendback.vercel.app/api/menu/menulist/${category._id}`)
        .then((response) => {
          setMenus(response.data);
        })
        .catch((error) => {
    ...
    const fetchHotelInfo = async () => {
      try {
        ...
        const allHotelsResponse = await axios.get(
          "https://backendback.vercel.app/api/hotel/get-all"
        );
        ...
        if (defaultHotelId) {
          ...
          const response = await axios.get(
            `https://backendback.vercel.app/api/hotel/get/${defaultHotelId}`
          );
                ...
                <div key={orderItem._id} className="flex items-center mb-3">
                  <div className="flex flex-row items-center ">
                    <div className="flex items-center h-full">
                      <img
                        // src={`https://backendback.vercel.app/${orderItem.imageUrl}`}
                        src={`/tray.png`}
                  ...
                  <div className="flex flex-col md:flex-row items-center">
                    ...
                    <input
                      ...
                      placeholder="Enter or Select Taste"
                    ...
                    {selectedTastes[orderItem._id] === "other" && (
                      <input
                        ...
                        className="mt-1 p-1 border rounded-md text-sm w-22   text-gray-500"
                        required
                      />
                    )}

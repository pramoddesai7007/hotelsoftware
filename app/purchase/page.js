    ...
    const fetchStockQty = async () => {
      try {
        const response = await axios.get(
          `https://backendback.vercel.app/api/purchase/purchase/stockQty?itemName=${formData.itemName}`
        );
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await axios.post(
        "https://backendback.vercel.app/api/purchase/purchase/savebill",
        data
      );
      ...
      if (parseFloat(data.total) > 0) {
        ...
        await axios.post("https://backendback.vercel.app/api/supplier/supplier/debit", {
          vendorName,
          amount: parseFloat(data.paidAmount),
        });
      } else {
        // If there is no paid amount, update the supplier's credit balance
        await axios.post("https://backendback.vercel.app/api/supplier/supplier/credit", {
          vendorName,

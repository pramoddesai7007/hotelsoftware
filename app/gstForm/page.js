...
const GSTForm = () => {
  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      await axios.delete(`https://backendback.vercel.app/api/gst/gst/${deletingItemId}`);
  ...
  const fetchGSTList = async () => {
    try {
      const response = await axios.get('https://backendback.vercel.app/api/gst/list');
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      await axios.post('https://backendback.vercel.app/api/gst/create', { gstPercentage });
      fetchGSTList();
      setGSTPercentage('');
    } catch (error) {
  ...
  return (
    <>
      ...
      <div className="max-w-5xl mx-auto mt-20 container shadow-md">
        <form onSubmit={handleFormSubmit} className="mb-4 flex items-center ml-3">
          ...
          <h1 className="text-xl font-bold font-sans mb-2 md:mb-0 text-orange-600">Purchase GST</h1>
            <label htmlFor="gstPercentage" className="block text-sm font-medium text-gray-600 mt-2">
              GST Percentage:
            </label>
            <input
              type="text"

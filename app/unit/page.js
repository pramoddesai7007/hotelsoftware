...
const UnitList = () => {
  ...
  useEffect(() => {
    const fetchUnits = async () => {
      try {
        const response = await axios.get('https://backendback.vercel.app/api/unit/units');
  ...
  const handleEditSubmit = async (e, updatedUnit) => {
    ...
    try {
      ...
      await axios.patch(`https://backendback.vercel.app/api/unit/units/${selectedUnit._id}`, {
  ...
  const handleDeleteSubmit = async () => {
    try {
      await axios.delete(`https://backendback.vercel.app/api/unit/units/${selectedUnit._id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post('https://backendback.vercel.app/api/unit/units', { unit: newUnit });
        ...
        <form onSubmit={handleSubmit} className="mb-4">
          <div className="mb-4">
            ...
            <input
              ...
              className="mt-1 p-2 border rounded-md lg:w-1/3 focus:outline-none focus:ring focus:border-blue-300"
            />
          </div>
          ...
          <div className="overflow-auto max-h-full">
            <table className="min-w-full ">
              <thead className="text-base bg-zinc-100 text-yellow-700 border">
                <tr>
                  <th className="p-2 text-left text-gray lg:pl-40 pl-4">Unit</th>
                  <th className="p-2 text-left text-gray lg:pl-96 pl-4">Actions</th>
                </tr>
              </thead>
              <tbody className="text-md font-sans font-semibold">
                {units.map((unit, index) => (
                  ...
                  >
                    <td className="p-2 text-left text-gray lg:pl-40 pl-4">{unit.unit}</td>
                    <td className=" pl-2 p-2 text-left text-gray lg:pl-96">
                      <button
